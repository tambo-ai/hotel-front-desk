      ...
      <div className="bg-card border border-border rounded-lg p-3">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          {["King", "Queen", "Suite"].map((type) => {
            const rate = rates.find(
              (r) => r.roomType === type && r.date === today,
            );
            return (
              <div
                key={type}
                className="bg-secondary/50 rounded p-2 text-center"
              >
        ...
        {stagedChange && (
          <div className="mt-2 text-xs text-amber-400">
            Pending: {stagedChange.roomType} ${stagedChange.previousRate} → $
            {stagedChange.newRate}
          </div>
        )}
      </div>
    );
        ...
        <table className="w-full">
          <thead className="bg-secondary/50">
            <tr>
              ...
              {(!roomType || roomType === "Queen") && (
                <th className="text-center text-xs font-medium text-muted-foreground uppercase tracking-wide px-4 py-3">
                  Queen
                </th>
              )}
          ...
          <tbody className="divide-y divide-border">
            {dates.map((d) => {
              ...
              const getRate = (type: RoomType) =>
                dateRates.find((r) => r.roomType === type);

              return (
                <tr
                  key={d}
                  className={`${isWeekend ? "bg-secondary/20" : ""} ${isToday ? "bg-blue-500/10" : ""}`}
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-foreground font-medium">
                          {dayOfWeek}
                          {isToday && (
                            <span className="ml-2 text-xs px-1.5 py-0.5 bg-blue-500 text-foreground rounded">
                              Today
                            </span>
                          )}
                        </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1 justify-center">
                        {dateRates[0]?.competitorRates?.map((comp) => (
                          <span
                            key={comp.name}
                            className="text-xs px-2 py-0.5 bg-secondary text-foreground/80 rounded"
                            title={comp.name}
                          >
                            ${comp.rate}
                          </span>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {stagedChange.roomType} on{" "}
                {new Date(stagedChange.date).toLocaleDateString()}:{" "}
                <span className="text-foreground/80">
                  ${stagedChange.previousRate}
                </span>
                {" → "}
                <span className="text-foreground">${stagedChange.newRate}</span>
              </p>
            </div>
            <button
...
function RateCell({
  ...
  rate:
    | { rate: number; competitorRates?: { name: string; rate: number }[] }
    | undefined;
  roomType: RoomType;
  date: string;
  stagedChange: {
  ...
  const avgCompetitor =
    rate?.competitorRates && rate.competitorRates.length > 0
      ? rate.competitorRates.reduce((sum, c) => sum + c.rate, 0) /
        rate.competitorRates.length
      : null;
  ...
  return (
    <div
      className={`relative ${isStaged ? "ring-2 ring-amber-400 rounded-lg" : ""}`}
    >
      <div className="flex items-center justify-center gap-1">
        <span
          className={`text-lg font-bold ${isStaged ? "text-amber-400" : "text-foreground"}`}
        >
          ${displayRate}
        </span>
        {comparison === "above" && (
      ...
      <div className="mt-1 flex gap-1 justify-center">
        <button
          ...
          className="text-xs px-2 py-0.5 bg-secondary hover:bg-secondary/80 text-foreground/80 rounded transition-colors"
        >
          -$10
        </button>
        <button
